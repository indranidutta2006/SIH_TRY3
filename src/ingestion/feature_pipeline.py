...
class FeatureEngineeringPipeline:
    ...
    def create_features(
        ...
        # 4. Kinematics
        # implied_hours = distance_nm / speed_knots
        # Derived purely from operational inputs (distance, service speed) — independent of
        # the source-specific reported hours_at_sea, which varies by data provenance.
        # NOTE: implied_speed (= D/H) and speed_discrepancy (= D/H - V) are INTENTIONALLY
        # ABSENT.  Both are invertible functions of hours_at_sea and would re-introduce the
        # duration proxy that the leakage fix is designed to remove.
        speed_safe_div = np.maximum(speed_safe, 1e-4)
        implied_hours = dist_safe / speed_safe_div
        df["implied_hours"] = implied_hours

        # Operational duration category (source_group) — derived from implied_hours (D/V),
        # NOT from the raw hours_at_sea field.
    ...
    def get_training_features_and_target(
        ...
        # Drop targets, identifiers, raw hours_at_sea, and duration-proxy features from X.
        # implied_speed (= D/H) and speed_discrepancy (= D/H - V) are explicitly listed here
        # as a safety guard: they are not generated by create_features(), but if that ever
        # changes, they must still be excluded — both are invertible functions of hours_at_sea.
        drop_cols = set(IDENTIFIER_COLUMNS).union(
            {
                "fuel_consumption", "co2_emissions",
                "hours_at_sea",           # raw source-specific duration — primary leakage vector
                "implied_speed",          # D / H  — invertible function of hours_at_sea
                "speed_discrepancy",      # D / H - V — invertible function of hours_at_sea
            }
        )
    ...
    def get_inference_features(
        ...
        # Also explicitly lists duration proxies as a safety guard (mirroring training path).
        drop_cols = set(IDENTIFIER_COLUMNS).union(
            {
                "fuel_consumption", "co2_emissions",
                "hours_at_sea",       # raw source-specific duration
                "implied_speed",      # D / H  — invertible function of hours_at_sea
                "speed_discrepancy",  # D / H - V — invertible function of hours_at_sea
            }
        )
